...
namespace JPushTest
{
    class JPushUtil
    {
         ...
         * extras.Add("Count",4);
         */
        public MessageResult sendNotification(HashSet<string> targets, TargetType targetType, string content,string title,Hashtable extras)  
        {
            ...
            //step2：准备告警内容
            pushPayload.notification = createNotification(content, title, extras);
        ...
        private Notification createNotification(string content, string title, Hashtable extras)
        {
            Notification not = new Notification();
            //统一制定设置通知内容，如果各平台单独指定，这里会被覆盖
            //not.setAlert(content);
            not.setAndroid(createAndroidNotification(content,title));
            not.setIos(createIosNotification(content,title));
            addExtras(not, extras);
            return not;
        }

        private void addExtras(Notification not,Hashtable extras)
        {
            foreach (DictionaryEntry extra in extras)
            {
                not.AndroidNotification.AddExtra(extra.Key.ToString(), extra.Value.ToString());
                not.IosNotification.AddExtra(extra.Key.ToString(), extra.Value.ToString());
            }
        }

        private IosNotification createIosNotification(string content,string title)
        {
            //定制ios效果
            IosNotification ios = new IosNotification();
            ios.setAlert(new IosAlert(content, title));
        ...
        private AndroidNotification createAndroidNotification(string content,string title)
        {   
            //定制android效果
            AndroidNotification android = new AndroidNotification();
            android.setAlert(content);
            android.setTitle(title);//如果指定了，则通知里原来展示App名称的地方，将展示成这个字段。
    ...
    public class IosAlert
    {
        ...
        public IosAlert(string content,string title)
        {
            this.body = content;
            this.title = title;
        }
